...
func AddTodoMobile(ctx *fiber.Ctx) error {
...
func GetTODOMobile(ctx *fiber.Ctx) error {
	cookie := ctx.Get("Authorization")
	fmt.Println(cookie)
...
func UpdateTodoMobile(ctx *fiber.Ctx) error {
	cookie := ctx.Get("Authorization")
...
func DeleteTodoMobile(ctx *fiber.Ctx) error {
	cookie := ctx.Get("Authorization")
	id, err := util.ParseJWT(cookie)
	if err != nil {
		ctx.Status(400)
